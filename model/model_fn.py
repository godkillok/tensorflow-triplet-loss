...
def build_model(is_training, images, params):
    ...
    """
    out = images #Tensor("Reshape:0", shape=(?, 28, 28, 1), dtype=float32)
    # Define the number of channels of each convolution
    # For each block, we do: 3x3 conv -> batch norm -> relu -> 2x2 maxpool
    num_channels = params.num_channels
    bn_momentum = params.bn_momentum
    channels = [num_channels, num_channels * 2] #<class 'list'>: [32, 64]
    ...
    assert out.shape[1:] == [7, 7, num_channels * 2] #Tensor("model/block_2/max_pooling2d/MaxPool:0", shape=(?, 7, 7, 64), dtype=float32)
...
def model_fn(features, labels, mode, params):
    ...
    images = features #Tensor("IteratorGetNext:0", shape=(?, 784), dtype=float32)
    images = tf.reshape(images, [-1, params.image_size, params.image_size, 1]) #Tensor("Reshape:0", shape=(?, 28, 28, 1), dtype=float32)
