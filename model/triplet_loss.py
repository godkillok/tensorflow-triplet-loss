...
def _pairwise_distances(embeddings_a,embedding_b, squared=False):
    ...
    tf.summary.histogram('square_norm', square_norm)
    #
...
def batch_all_triplet_loss(labels, embeddings_a,embedding_b, margin, squared=False):
    ...
    triplet_loss=triplet_loss-0.05*tf.reduce_mean(square_norm)+0.005*tf.reduce_mean(pairwise_dist-tf.matrix_diag(square_norm))
    # triplet_loss= tf.maximum(0.,triplet_loss)
